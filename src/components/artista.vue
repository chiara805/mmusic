    ...
    <div>
        <div style="margin: 4vh 2vw 4vh 2vw;">
          ...
          <button @click="this.sezione__centrale = 1; this.artworkSearch()"
            :class="{ 'buttonlight': this.sezione__centrale == 0, 'buttonheavy': this.sezione__centrale == 1 }">Canzoni salvate</button>
        </div>
            </div>
        </div>
        <div>
      ...
      <v-slide-group v-model="model" class="pa-4" selected-class="bg-success" show-arrows>
        <v-slide-group-item v-for="music in this.musicByArtist" v-slot="{ isSelected, toggle, selectedClass }">
          <div class="ods__card ods__card__little" @click="$router.push({ path: '/canzone:' + music.title__intero})"
            :style="{ backgroundImage: 'linear-gradient(to bottom, rgba(255, 255, 255, 0), rgba(201, 201, 201, 0.73)), url(' + music.image + ')' }">
...
export default {
    ...
    methods: {
        ...
        async musicSearch(){
            ...
            try {
          const response = await DataService.getMusicbyArtist(this.$route.params.arId.substring(1));
          
          this.musicByArtist = response.data._embedded.music.map((result) => (
          {
            id: result._links.self.href.replace("https://www.music.net/music/", ""),
            title: result.title,
            desc: result.description,
            type: result.type,
            image: result._links.thumbnail.href,
            title__intero: result._links.permalink.href.replace("https://www.music.net/music/", ""),
          })
        );

        console.log(this.musicByArtist)
