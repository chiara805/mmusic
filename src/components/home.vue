<template>
  ...
  <h3>Scopri le canzoni più ascoltate</h3>
  </div>
  <div class="container__home__gene" v-for="(gene, index) in this.arrayGeneRes" :key="gene.id">
    ...
    <v-sheet class="mx-auto">
      <v-slide-group v-model="model" class="pa-4" selected-class="bg-success" show-arrows>
        <v-slide-group-item v-for="music in this.arrayGeneRes[index]" :key="music.link" v-slot="{ isSelected, toggle, selectedClass }">
...
/*           if(DataService.getGene(art.title__intero).total_count >0){
            this.arrayGeneRes[i].push({
              id: element._links.self.href.replace("https://www.music.net/music/", ""),
              title: element.title,
              desc: element.description,
              type: element.type,
              image: element._links.thumbnail.href,
              style: this.ArrayGenes[i],
              title__intero: element._links.permalink.href.replace("https://www.music.net/music/", ""),
            })
          } */
    }
  },
