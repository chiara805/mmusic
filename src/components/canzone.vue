<template>
    <div class="container__interno" v-if="this.MusicObj != undefined">
      ...
      <section class="ods__mini__card" >
        ...
        <div>
          <h4>Titolo</h4>
          <h1 class="h1__large">{{ MusicObj.title}}</h1>
        </div>
        <div>
          <h4>Descrizione</h4>
          <h2>{{ MusicObj.description}}</h2>
        </div>
        <div>
          <h4>Tipologia</h4>
          <h2>{{ MusicObj.type}}</h2>
...
export default {
  ...
  methods: {
    ...
    checkIfLiked: async function () {
            const docRefUt = doc(DataService.dbEx(), 'likes', this.user + "_oppla_" + this.$route.params.caId.substring(1));
        ...
        unlikeImage: async function () {
            deleteDoc(doc(DataService.dbEx(), 'likes', this.user + "_oppla_" + this.$route.params.caId.substring(1))).then(() =>{
                this.checkIfLiked()
            });
