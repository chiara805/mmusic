...
<!--           <div>
      ...
      <div class="profilo__centro">
        <div class="gallery-image">
          <div v-for="post in this.arraySalvati" class="ods__card ods__card__little img-box" @click="$router.push({ path: '/canzone:' + post.title__intero})"
    ...
    takeUserPost: async function () {
      ...
      querySnapshot.forEach((doc) => {
        ...
        if (doc.data().userId == route__nodot) {
          this.musicData(doc.data().postId)
        }
      });
      console.log(this.arraySalvati)
    },
    async musicData(title) {
      try {
        const response = await DataService.getGene(title);
        this.arraySalvati.push(
        {
          id: response.data._embedded.results[0]._links.self.href.replace("https://www.music.net/music/", ""),
          ...
          title__intero: response.data._embedded.results[0]._links.permalink.href.replace("https://www.music.net/music/", ""),
        }
